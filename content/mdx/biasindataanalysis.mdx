---
lessonId: biasindataanalysis
courseId: ml-engineering
chapterId: chapter-1
lessonSlug: biasindataanalysis
...
Computers, data, and [algorithms](/learn) are not actually completely objective. It is true that data analysis can help us make better decisions, but it is not immune to bias. Humans create technologies and algorithms. As a result, they often have human biases encoded into them. It’s clear that we need to pay attention to other information streams (our eyes and ears) when we drive with GPS. Similarly, we need to look at more information sources when we evaluate data analysis results or reports.
